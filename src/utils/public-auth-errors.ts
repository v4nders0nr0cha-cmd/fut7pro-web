...
export function getHumanAuthErrorMessage(
  ...
  if (
    ...
    message.includes("não possui conta global") ||
    message.includes("nao possui conta global") ||
  ...
  if (message.includes("solicita") && message.includes("pendente")) {
    return "Sua solicitação para entrar neste grupo já foi enviada. Aguarde a aprovação do administrador.";
  }

  if (message.includes("ja faz parte") || message.includes("já faz parte")) {
    return "Você já faz parte deste grupo. Entre para acompanhar partidas, estatísticas e informações do seu perfil.";
  }

  if (message.includes("aprovad")) {
    return "Sua entrada neste grupo foi aprovada. Agora você já pode acessar sua área de atleta.";
  }
