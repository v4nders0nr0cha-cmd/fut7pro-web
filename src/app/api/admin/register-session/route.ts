...
export async function POST(req: NextRequest) {
  ...
  if (!user?.accessToken) return jsonResponse("Não autorizado.", 401);
  ...
  if (!adminEmail) return jsonResponse("E-mail do administrador não encontrado.", 400);
  ...
  if (!payload.rachaSlug?.trim() || !SLUG_REGEX.test(payload.rachaSlug.trim()))
    return jsonResponse("Slug inválido: use minúsculas, números e hifens (3-30).", 400);
  if (RESERVED_SLUGS.has(payload.rachaSlug.trim().toLowerCase()))
    return jsonResponse("Slug inválido: reservado.", 400);
  ...
  const res = await fetch(resolvePath(baseUrl, "/auth/session/tenant"), {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
