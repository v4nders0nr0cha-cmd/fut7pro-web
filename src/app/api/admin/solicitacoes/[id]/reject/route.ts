...
const REJECT_ENDPOINT = "/admin/solicitacoes";
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const url = `${base}${REJECT_ENDPOINT}/${encodeURIComponent(params.id)}/reject`;
  ...
  try {
    ...
    return jsonResponse(
      { error: "Falha ao rejeitar solicitacao", details: message },
      { status: 500 }
    );
  }
}
