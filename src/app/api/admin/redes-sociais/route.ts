...
import { buildSocialLinksPaths, triggerPublicRevalidate } from "../../_proxy/revalidate";
...
async function forwardToBackend(
  ...
  const targetUrl = new URL(`${getApiBase()}/admin/redes-sociais`);
...
export async function PUT(req: NextRequest) {
  ...
  return forwardToBackend(
    ...
    true,
    async ({ tenantSlug }) => {
      await triggerPublicRevalidate(tenantSlug, buildSocialLinksPaths(tenantSlug));
    }
  );
}
