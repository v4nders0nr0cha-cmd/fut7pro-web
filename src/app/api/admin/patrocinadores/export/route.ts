...
export async function GET(req: NextRequest) {
  ...
  if (!user || !user.accessToken) {
    return toJson({ error: "Não autenticado" }, { status: 401 });
  }
  ...
  if (!tenantSlug) {
    return toJson({ error: "Tenant slug obrigatório" }, { status: 400 });
  }

  const backendUrl = new URL("/sponsors/export", getApiBase());
  ...
  try {
    ...
    if (!backendResponse.ok) {
      ...
      try {
        ...
        return toJson(
          {
            error: "Falha ao exportar patrocinadores",
