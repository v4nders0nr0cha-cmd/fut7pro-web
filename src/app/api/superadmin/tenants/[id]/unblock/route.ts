...
export async function POST(req: NextRequest, { params }: { params: { id?: string } }) {
  ...
  if (payload?.confirmed !== true || !reason) {
    return jsonResponse(
      { error: "Confirmacao explicita e motivo sao obrigatorios para desbloquear racha." },
      { status: 400 }
    );
  }

  const body = JSON.stringify({ reason });
  const targetUrl = `${getApiBase()}/superadmin/tenants/${encodeURIComponent(id)}/unblock`;
