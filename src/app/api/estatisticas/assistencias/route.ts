...
export async function GET(request: Request) {
  if (isProd && isWebDirectDbDisabled) {
    return NextResponse.json(
      { error: "web_db_disabled: use a API do backend para estatisticas/assistencias" },
      { status: 501 }
    );
  }
  ...
  try {
    ...
    console.error("GET /api/estatisticas/assistencias failed", error);
    return NextResponse.json({ error: "failed to compute assists" }, { status: 500 });
  }
}
