...
export async function GET(
  ...
  const url = new URL(
    `${backendBase.replace(/\/+$/, "")}/public/${encodeURIComponent(
      params.slug
    )}/athletes/${encodeURIComponent(params.athleteSlug)}/premium-profile`
  );
  ...
  try {
    ...
    if (!res.ok) {
      return json(
        { error: "Falha ao consultar perfil premium", status: res.status, body },
        { status: res.status }
      );
    }
    ...
    return json({ error: "Falha ao consultar perfil premium", details: message }, { status: 500 });
  }
}
