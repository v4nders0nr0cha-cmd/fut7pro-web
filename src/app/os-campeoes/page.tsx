...
export default function ClientWrapper() {
  ...
  return (
    <div className="min-h-screen bg-fundo text-white px-4 pt-4 pb-10">
        ...
        {campeoesAno
          ...
          .map((item, idx) => (
            <CampeaoAnoCard
              key={idx}
              {...item}
              href={item.slug ? `/atletas/${item.slug}` : item.href}
            />
          ))}
      </div>
      ...
      {dadosQuadrimestre ? (
        <QuadrimestreGrid dados={dadosQuadrimestre} />
      ) : (
        <p className="text-center text-gray-400">Nenhum dado disponível para o ano selecionado.</p>
      )}
    </div>
  );
}
