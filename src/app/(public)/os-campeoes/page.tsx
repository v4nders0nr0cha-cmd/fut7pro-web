...
function formatDate(value: string | null | undefined) {
  ...
  try {
    return parsed.toLocaleDateString("pt-BR", { year: "numeric", month: "short", day: "numeric" });
  } catch (_error) {
    ...
    return (
      <div className="w-full max-w-[1440px] mx-auto px-1 pt-[40px] pb-10">
        <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-6">
          ...
          <p className="text-red-300">{error ?? "Tente novamente em instantes."}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {campeoes.map((campeao) => {
            ...
            return (
              <div
                key={campeao.id}
                className="bg-[#1A1A1A] rounded-2xl p-6 text-white hover:shadow-lg transition-all border border-white/5"
              >
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <p>
                    <span className="font-semibold">Data:</span> {formatDate(campeao.data)}
                  </p>
                  {campeao.descricao && (
                    <p>
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {partidas.slice(0, 6).map((partida) => (
              <div
                key={partida.id}
                className="bg-[#1A1A1A] rounded-xl p-4 text-white border border-white/5"
              >
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm text-textoSuave">{formatDateTime(partida.data)}</span>
                  <span
