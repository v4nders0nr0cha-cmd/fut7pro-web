...
- Estado geral: parcialmente alinhado à especificação; pronto para demos, não pronto para vendas em escala.
...
- Gaps críticos: uso de Prisma no web em produção (contraria a especificação), módulos centrais com mocks (patrocinadores), cadastros públicos/fluxos sem delegar ao backend, relatórios/exportações incompletos, notificações/engajamento sem integração real.
...
- Core
  ...
  - Rankings & Estatísticas: parcialmente implementado (agregadores e algumas rotas; faltam títulos automáticos e filtros completos em todas telas).
...
- Monetização
  - Patrocinadores: parcial (UI presente; mocks; ausência de métricas/UTM/exportações reais).
- Enterprise
...
- Remover mocks (sorteio, patrocinadores, destaques, dashboard) e integrar dados reais.
...
- Inconsistências de consumo de API: hooks/rotas locais versus backend.
- Mocks em componentes críticos (sorteio, patrocinadores, destaques, etc.).
...
- [ ] Fase 1 - Dados Reais (3-5 dias)
  ...
  - [ ] Patrocinadores: CRUD básico integrado às rotas `/api/admin/patrocinadores` (Next.js API, Prisma dev-only), substituindo mocks no painel. Em andamento.
